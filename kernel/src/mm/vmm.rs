...
impl Vmm {
    pub fn new(top_level: PAddr, hhdm_offset: u64) -> Self {
        Self {
            top_level,
            hhdm_offset,
        }
...
fn clone_table(src_phys: PAddr, hhdm: u64, level: u8) -> PAddr {
    let dst_frame = pmm::alloc_frame().expect("clone_table: out of frames");
    ...
    for i in 0..PAGE_SIZE_ENTRIES {
        ...
        if is_leaf {
            ...
            let new_child = clone_table(child_phys, hhdm, level - 1);
